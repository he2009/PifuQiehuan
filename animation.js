...
(function(newDuilib) {
	...
	newDuilib.TimeStep = (function() {
		...
		TimeStep.prototype.update = function(delta) {
			...
			if (isArray) {
				this.current = [newDuilib.lerp(start[0], end[0], this.percent), newDuilib.lerp(start[1],
					end[1], this.percent)];
			} else {
	...
	newDuilib.APNode = (function() {
		...
		APNode.prototype.scaleTo = function(scale, duration) {
			if (scale != undefined) {
				this.updateTimeStep('scale', (this.scale == undefined ? 1 : this.scale), scale,
					duration);
				this.scale = scale;
			}
		...
		APNode.prototype.rotateTo = function(angle, duration) {
			if (angle != undefined) {
				this.updateTimeStep('angle', (this.angle == undefined ? 0 : this.angle), angle,
					duration);
				this.angle = angle;
			}
		...
		AnimationPlayer.prototype.getSpineBounds = function(filename) {
			...
			if (!this.resized) {
				...
				if (this.dprAdaptive == true)
					dpr = Math.max(window.devicePixelRatio * (window.documentZoom ? window
						.documentZoom : 1), 1);
	...
	newDuilib.AnimationPlayerPool = (function() {
		function AnimationPlayerPool(size, pathPrefix, thisName) {
			...
			for (var i = 0; i < this.animations.length; i++) this.animations[i] = new newDuilib
	...
	newDuilib.DynamicPlayer = (function() {
		...
		function DynamicPlayer(pathPrefix) {
			this.id = newDuilib
				.BUILT_ID++; // 这个是全局的Build_id. 用来表示这个动态皮肤播放对象的id, 暂时只看到new DynamicPlayer对象才会导致newDuilib.BUILT_ID++
			this.dpr = 1;
			this.width = 120;
			this.height = 180;
			...
			if (offscreen) {
				...
				for (var i = 0; i < workers.length; i++) {
					...
					workers[i].postMessage({
						...
						dpr: Math.max(window.devicePixelRatio * (window.documentZoom ? window
							.documentZoom : 1), 1),
						modifyQhlxPreview: skinSwitch.lib.config[skinSwitch.configKey
							.modifyQhlxPreview]
					}, [canvas]);
