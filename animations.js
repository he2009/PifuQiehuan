...
class BaseAnimation {
	...
	getSpineActions(filename, toLoadActions) {
		...
		for (let i = 0; i < actions.length; i++) result[i] = {
			name: actions[i].name,
...
class Animation3_6 extends BaseAnimation {
	...
	playSpine(sprite, position) {
		...
		var entry = skeleton.state.setAnimation(0, sprite.action ? sprite.action : skeleton.defaultAction, sprite
			.loop);
		entry.mixDuration = 0;
	...
	render(timestamp) {
		...
		for (var i = 0; i < nodes.length; i++) {
			...
			if (sprite.viewportNode) {
				...
				// 将当前视口选在小窗的位置上
				gl.viewport((rect.x - canvasRect.x) * dpr, (canvasRect.bottom - rect.bottom) * dpr, width * dpr,
					height * dpr);
				gl.scissor((rect.x - canvasRect.x) * dpr, (canvasRect.bottom - rect.bottom) * dpr, width * dpr,
					height * dpr);
			}
...
class Animation4_0 extends BaseAnimation {
		...
		if (skelType === 'json') {
			thisAnim.spine.assetManager.loadText(filename + '.json', () => {
				thisAnim.spine.assetManager.loadTextureAtlas(filename + '.atlas', () => {
					thisAnim.spine.assets[filename] = {
						name: filename,
						skelType: skelType
					};
					if (onload) onload()
	...
	prepSpine(filename, autoLoad) {
		...
		skeleton.bounds = {
			offset: new spine_4.Vector2(),
			size: new spine_4.Vector2()
		}
		skeleton.getBounds(skeleton.bounds.offset, skeleton.bounds.size, []);
		skeleton.defaultAction = data.animations[0].name;
		skeleton.node = undefined;
...
class Animation4_1 extends BaseAnimation {
	...
	prepSpine(filename, autoLoad) {
		...
		skeleton.bounds = {
			offset: new this.spineLib.Vector2(),
			size: new this.spineLib.Vector2()
		}
		skeleton.getBounds(skeleton.bounds.offset, skeleton.bounds.size, []);
		skeleton.defaultAction = data.animations[0].name;
		skeleton.node = undefined;
...
class Animation3_8 extends BaseAnimation {
	...
	prepSpine(filename, autoLoad) {
		...
		skeleton.bounds = {
			offset: new spine3_8.Vector2(),
			size: new spine3_8.Vector2()
		};
		skeleton.getBounds(skeleton.bounds.offset, skeleton.bounds.size, []);
		skeleton.defaultAction = data.animations[0].name;
		skeleton.node = undefined;
		...
		switch (version) {
			case SupportSpineVersion.v3_6:
				if (!this.animations[version]) {
					this.animations[version] = new Animation3_6(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
					})
				}
			...
			case SupportSpineVersion.v4_0:
				if (!this.animations[version]) {
					this.animations[version] = new Animation4_0(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
			...
			case SupportSpineVersion.v3_8:
				if (!this.animations[version]) {
					this.animations[version] = new Animation3_8(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
			...
			case SupportSpineVersion.v3_5_35:
				if (!this.animations[version]) {
					this.animations[version] = new Animation3_5_35(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
			...
			case SupportSpineVersion.v3_7:
				if (!this.animations[version]) {
					this.animations[version] = new Animation3_7(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
			...
			case SupportSpineVersion.v4_1:
				if (!this.animations[version]) {
					this.animations[version] = new Animation4_1(this.pathPrefix, this.canvas, this.dpr, this
						.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
			...
			default:
				if (!this.animations[SupportSpineVersion.v3_6]) {
					this.animations[SupportSpineVersion.v3_6] = new Animation3_6(this.pathPrefix, this.canvas, this
						.dpr, this.offscreen)
					this.animations[version].update({
						width: this.width,
						height: this.height
	...
	constructor(animation, pathPrefix, lib) {
		...
		lib.arenaReady.push(function() {
			_this.worker.postMessage({
				...
				dpr: Math.max(window.devicePixelRatio * (window.documentZoom ? window.documentZoom :
					1), 1)
			})
			// 提前加载
...
class DecadeAnimationProxy {
	...
	constructor(animation, lib) {
		...
		this.am = new AnimationManager(lib.assetURL + 'extension/十周年UI/assets/animation/', animation.canvas,
			88888, )
		this.cap = animation.cap
		this.nameVersionMap = {}
