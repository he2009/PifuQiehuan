...
importScripts('spine.js', './spine-lib/spine_4_0_64.js', './spine-lib/spine_3_8.js',
	'./spine-lib/spine_3_5_35.js', './spine-lib/spine_3_7.js', './spine-lib/spine_4_1.js', 'animation.js',
	'settings.js', 'animations.js');
let window = self;
...
class PlayerAnimation {
	...
	constructor(data) {
		...
		this.animationManager = new AnimationManager(data.pathPrefix, data.canvas, 989898, {
			dpr: data.dpr
		})
	...
	preLoadPlayerSkel(data) {
		...
		for (let act of [{
				...
				json: player.json,
				alpha: player.alpha
			}, player.gongjiAction, player.teshuAction, player.chuchangAction]) {
	...
	playAction(data) {
		...
		if (!this.getAnni(player, actionParams.version).hasSpine(actionParams.name)) {
			this.getAnni(player, actionParams.version).loadSpine(actionParams.name, actionParams.json ? 'json' :
				'skel', () => {
	...
	playZhiShiXianWithPlayNode(playNode, animation, data) {
		...
		if (dy) {
			...
			this.playZhiShiXian(playNode.player, playNode.actionParams.attackArgs, data, animation.showTime * (
				delay * 1000 || 0), playNode)
		}
					...
					setTimeout(() => {
						...
						let playBaoZha = () => {
							for (let p of attackArgs.targets) {
								...
								this.getAnni(player, sprite.version).playSpine(sprite, {
									referNode: referNode
								})
							}
						}
						...
						let playZhishixian = () => {
							...
							let getStartXY = () => {
								...
								if (playNode) {
									if (dy.startPos === 'player') {
										...
										return {
											x1: attackArgs.bodySize.bodyWidth * dy.startPos.x[1] +
												dy.startPos.x[0],
											y1: attackArgs.bodySize.bodyHeight - attackArgs.bodySize
												.bodyHeight * dy.startPos.y[1] - dy.startPos.y[0],
										}
									} else {
										if (Array.isArray(playNode.x)) {
											x1 = attackArgs.bodySize.bodyWidth * playNode.x[1] +
												playNode.x[0];
										} else {
											x1 = playNode.x;
										}
										...
										if (Array.isArray(playNode.y)) {
											y1 = attackArgs.bodySize.bodyHeight - attackArgs
												.bodySize.bodyHeight * playNode.y[1] - playNode.y[0]
										} else {
							...
							for (let p of attackArgs.targets) {
								...
								let node = this.getAnni(player, dy.version).playSpine(sprite, {
									x: startX,
									y: attackArgs.bodySize.bodyHeight - startY
								})
							...
							if (dy.effect) {
								...
								setTimeout(() => {
										if (!this.getAnni(player, dy.effect.version).hasSpine(dy
												.effect.name)) {
											this.getAnni(player, dy.effect.version).loadSpine(dy
												.effect.name, dy.effect.json ? 'json' : 'skel',
												playBaoZha)
										} else {
											playBaoZha()
										}
									}, zhishixianTime / (dy.speed || 1) * 1000 * dy.effect.delay ||
									0.5)
							}
						}

						if (!this.getAnni(player, dy.version).hasSpine(dy.name)) {
							this.getAnni(player, dy.version).loadSpine(dy.name, dy.json ? 'json' : 'skel',
								playZhishixian)
						} else {
							playZhishixian()
						}
	...
	lianxuChuKuang(player, actionParams, data) {
		...
		if (!this.getAnni(playNode.player, actionParams.version).nodes.includes(playNode)) {
			...
			return this.playChuKuangSpine(playedSprite, {
				showTime: actionParams.showTime
			}, data)
		}
		...
		if (actionParams.triggername !== 'useCardBefore' && data.action === 'GongJi' && playNode.actionParams
			.attackArgs) {
		...
		if (!player.shizhounian) {
			...
			if ((Math.abs(playNode.renderY - data.player.y)) > data.player.height / 3 * 2 || Math.abs(playNode
					.renderX - data.player.x) > data.player.width / 3 * 2) {
				notSetPos = true
			}
		}
		this.playChuKuangSpine(playNode, {
			showTime: lastTime,
			playRate: entry.trackTime / lastTime
		}, data, notSetPos)
	}
	...
	playChuKuang(player, actionParams, data) {
		...
		this.playChuKuangSpine(playedSprite, {
			showTime: actionParams.showTime
		}, data);
	}
	...
	completeParams(player) {
		...
		if (chuchang) {
			if (chuchangType === 'object') {
				if (!chuchang.name) {
					chuchang.name = player.name
				}
				if (!chuchang.action) {
					chuchang.action = 'play'
				}
				if (!chuchang.scale) {
					chuchang.scale = player.scale
				}
		...
		if (player.pos) {
			...
			// 如果是简单的设置为true, 那么说明当前动皮是静态皮肤, 也想出框, 那么和上面一样, 播放当前待机动作到中央
				...
				if (!gongjiAction.scale) {
					gongjiAction.scale = player.scale
				}
			...
			if (teshuType === 'string') {
				...
				if (teshuAction.name !== player.name) {
					...
					if (!teshuAction.x) {
						teshuAction.x = player.x
					}
					if (!teshuAction.y) {
						teshuAction.y = player.y
					}
				}
				if (!teshuAction.scale) {
					teshuAction.scale = player.scale
				}
			} else {
...
function setPos(apnode, data) {
	...
	if (data.me) {
		...
		if (data.direction.isLeft) data.player.x += 85;
		else data.player.x += 40;
	}
}
...
function completePlayerParams(avatarPlayer, action) {
	if (avatarPlayer) {
		if (!avatarPlayer.actionState) {
			avatarPlayer.actionState = {}
		} else {
...
function isChuKuang(data) {
	...
	if (playerState) {
		if (data.action === 'chuchang') {
			...
			if (playerState.lastAction === 'chuchang' && playerState.time && new Date().getTime() - playerState.time <=
				200) {
				return
			}
	...
	let checkNoChukuang = (player) => {
		...
		if (actionParams && actionParams.ck === false) {
			if (player.zhishixian && extraParams) {
				playerAnimation.playZhiShiXian(player, extraParams.attackArgs, data, (player.zhishixian.delay *
					1000) || 0)
			}
			postMessage({
				id: data.id,
	...
	if (completePlayerParams(primaryPlayer, data.action)) {
		if (!playerState) {
			playerAnimation.playerState[data.id] = {
				time: new Date().getTime(),
	...
	if (primaryPlayer && primaryPlayer.zhishixian && extraParams) {
		return playerAnimation.playZhiShiXian(primaryPlayer, extraParams.attackArgs, data, (primaryPlayer.zhishixian
			.delay * 1000) || 0)
	}
	if (deputyPlayer && deputyPlayer.zhishixian && extraParams) {
		return playerAnimation.playZhiShiXian(deputyPlayer, extraParams.attackArgs, data, (primaryPlayer.zhishixian
			.delay * 1000) || 0)
	}
...
function createDecadeAni(data) {
	decadeUIAni = new AnimationManager(data.pathPrefix, data.canvas, null, {
		dpr: data.dpr
	})
}
		...
		case 'playSpine':
			(() => {
				...
				if (dynamic.hasSpine(sprite.name)) {
					...
					dynamic.loadSpine(sprite.name, sprite.json ? 'json' : 'skel', () => {
						if (sprite.version != null && sprite.version !== '3.6' && !(sprite.version in
								decadeUIAni.aniVersionMap)) {
...
function loadResources(data) {
	...
	if (players) {
		for (let playerParams of players) {
			playerAnimation.preLoadPlayerSkel({
				player: playerParams
			})
		}
