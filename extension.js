	...
	return {
		...
		content: function(config, pack) {
			...
			function dynamicInit() {
				...
				if (!lib.config[skinSwitch.decadeKey.enable] && !lib.config[skinSwitch.decadeKey
						.dynamicSkin]) {
						...
						for (let m in skinSwitch.saveSkinParams[k]) {
							if (decadeUI.dynamicSkin[k] && decadeUI.dynamicSkin[k][m]) {
								...
								if (skinSwitch.saveSkinParams[k][m].gongji) {
									...
									if (gongji) {
										gongji = Object.assign(gongji, skinSwitch.saveSkinParams[k][m]
											.gongji)
									}
								...
								for (let assignK of ['x', 'y', 'scale', 'angle']) {
									if (skinSwitch.saveSkinParams[k][m][assignK] != null) {
										decadeUI.dynamicSkin[k][m][assignK] = skinSwitch.saveSkinParams[k][
											m][assignK]
									}
								}
								if (decadeUI.dynamicSkin[k][m].beijing && skinSwitch.saveSkinParams[k][m]
									.beijing) {
									decadeUI.dynamicSkin[k][m].beijing = Object.assign(decadeUI.dynamicSkin[
										k][m].beijing, skinSwitch.saveSkinParams[k][m].beijing)
								}
					...
					if (lib.config[skinSwitch.decadeKey.dynamicSkin]) {
						if (self.OffscreenCanvas === undefined) {
							...
							if (true) {} else {
								lib.element.player.logSkill = function(name, targets, nature, logv) {
									...
									if (game.phaseNumber > 0) {
										if (name.indexOf("_") !== 0 && skinSwitch.filterSkills.indexOf(
												name) === -1 || this.skills.indexOf(name) !== -1) {
											...
											if (this.isAlive() && this.dynamic && !this.GongJi) {
												skinSwitch.chukuangWorkerApi.chukuangAction(this,
													'TeShu')
											}
											let skillInfo = get.info(name)
											if (skillInfo) {
												let specailEvent = (player, triggerName) => {
													let res = skinSwitch.dynamic.getSpecial(player,
														triggerName)
													res.forEach(r => {
														const {
															avatar,
														...
														tryPlayTransform()
														// 检查是否有播放特效
														let effectPlay = special.condition[
															triggerName].play
														if (effectPlay) {
														...
														if (!audio) audio = special
															.condition[triggerName].audio
														if (audio) {
															game.playAudio('..', skinSwitch
																.dcdPath,
																'assets/dynamic', audio)
														}
									...
									if (lib.translate[name]) {
										...
										if (Array.isArray(targets) && targets.length) {
											...
											game.log(this, '对', str, '发动了', '【' + get.skillTranslation(
												name, this) + '】');
										} else {
											game.log(this, '发动了', '【' + get.skillTranslation(name,
												this) + '】');
										}
									}
									if (nature != false) {
									...
									if (info && info.ai && info.ai.expose != undefined &&
										this.logAi && (!targets || targets.length != 1 || targets[0] !=
											this)) {
							...
							lib.skill._gj = {
								// 指定多个目标也让触发攻击状态
								trigger: {
									player: ['useCardBefore', 'useCard1', 'useCard2']
								},
								forced: true,
								filter: function(event, player) {
									...
									return ((type == 'basic' || type == 'trick') && get.tag(event
										.card, 'damage') > 0)
								},
								content: function() {
									// player.GongJi = true;
									...
									if (!res || !res.dynamic) return player.GongJi = false;
										...
										let dy = (player.dynamic.primary && player.dynamic.primary
												.player && player.dynamic.primary.player.zhishixian
												) || (player.dynamic.deputy && player.dynamic.deputy
												.player && player.dynamic.deputy.player.zhishixian)
										let args = null

										let getArgs = (filterPlayers = []) => {
											if (dy != null) {
												if (event.triggername !== 'useCardBefore' &&
													event._trigger.targets.length < 2) {
													return
												}
												...
												args = {
													...
													targets: [], // 攻击目标坐标
													bodySize: {
														bodyWidth: decadeUI.get.bodySize()
															.width,
														bodyHeight: decadeUI.get.bodySize()
															.height
													}
												}
										...
										let timeDelta = player.__lastGongji ? new Date().getTime() -
											player.__lastGongji.t : 10000
										// 间隔极短的连续攻击忽略不计
										if (timeDelta < 20) return

										if (timeDelta >= 200) {
											if ( /*timeDelta <= 350 &&*/ event.triggername !==
												'useCardBefore') {
												if (player.__lastGongji && event._trigger.targets
													.length <= player.__lastGongji.tLen) {
													return
												}
											}
											getArgs()
											skinSwitch.chukuangWorkerApi.chukuangAction(player,
												'GongJi', args ? {
													attackArgs: args,
											...
											if (event.triggername !== 'useCardBefore') {
												if (event._trigger.targets.length <= player
													.__lastGongji.tLen) {
													return
												}
												getArgs()
												if (args) {
													skinSwitch.chukuangWorkerApi.chukuangAction(
														player, 'GongJi', {
															attackArgs: args,
															triggername: event.triggername
														})
							...
							lib.skill._hf = {
								...
								content: function() {
									...
									div.addEventListener(lib.config.touchscreen ? 'touchend' :
										'click',
										function() {
											skinSwitch.dynamic.skinDivShowOrHide(true);
											game.playAudio("..", "extension", "皮肤切换/audio/game",
												"Menu.mp3");
										})
								...
								lib.skill._initPlayerChangeSkin = {
									...
									content: function() {
										for (let p of game.players) {
											...
											img.src = lib.assetURL +
												'extension/皮肤切换/images/other/change-model.png'

											let timer
											p.node.avatar.addEventListener(lib.config.touchscreen ?
												'touchend' : 'click', (e) => {
													// 过滤掉选择角色事件
													if ((_status.event.name !== "chooseToUse" &&
															_status.clicked !== false) || p
														.classList.contains('selectable') || p
											...
											if (p.node.avatar2) {
												p.node.avatar2.addEventListener(lib.config
													.touchscreen ? 'touchend' : 'click', (
													e) => {
														// 防止选择事件触发
														if ((_status.event.name !==
																"chooseToUse" && _status
																.clicked !== false) || p
															.classList.contains('selectable') ||
															p.classList.contains('target')) {
															return
														}
														if (timer) {
															// 表示是双击事件
														...
														timer = setTimeout(() => {
															...
															skinHoverBtn.isPrimary =
																false
														}, 250)
													})
											}
											skinHoverBtn.addEventListener(lib.config.touchscreen ?
												'touchend' : 'click', (e) => {
													// e.stopPropagation()
													if (_status.playOpenTool) {
														if (_status.playOpenTool !== p) _status
															.playOpenTool
															.getElementsByClassName(
																'playerToolBox')[0].classList
															.add('hidden')
													}
													delete _status.playOpenTool
													box.classList.add('hidden')
													skinSwitch.qhly_open_small(skinHoverBtn
														.name, p, skinHoverBtn.isPrimary)

												})
										}

										document.addEventListener(lib.config.touchscreen ?
											'touchend' : 'click', () => {
												if (_status.playOpenTool) {
													_status.playOpenTool.getElementsByClassName(
														'playerToolBox')[0].classList.add(
														'hidden')
													delete _status.playOpenTool
												}
											})
							...
							lib.skill._ts = {
								...
								content: function() {
									...
									if (skillInfo) {
										if (skillInfo.limited) {
											...
											res.forEach(r => {
												...
												let pName = isPrimary ? player.name : player
													.name2
												...
												let tryPlayTransform = () => {
													...
													if (newName) {
														// 分割名字, 获取骨骼, 与当前角色的骨骼的名字比较,是否是同名
														let [key, skinName] = newName
															.split('/')
														let dInfo = key && skinName &&
															dskins[key] && dskins[key][
																skinName
															]
														if (dInfo) {
															skinSwitch.dynamic
												...
												tryPlayTransform()
												tryPlayEffect()
												if (!audio) audio = special.condition
													.xiandingji.audio
												if (audio) {
													game.playAudio('..', skinSwitch.dcdPath,
														'assets/dynamic', audio)
												}
											})
									...
									if (!player.doubleAvatar) {
										let teshu = player.dynamic.primary && player.dynamic.primary
											.player.teshu
							...
							lib.skill._playAudioToQueue = {
								...
								filter: function(event, player) {
									...
									return player.dynamic.primary && player.dynamic.primary.player
										.audio
								},
								content: function() {
									...
									let cardName
									if (card.name == 'sha' && (card.nature == 'fire' || card
											.nature == 'thunder' || card.nature == 'ice' || card
											.nature == 'stab')) {
							...
							lib.skill._checkDcdChuChang = {
								...
								forced: true,
								filter: function(event, player) {
									return game.players.length > 1 /*&&player.phaseNumber===0*/ &&
										player === event.player && !player.doubleAvatar && player
										.dynamic && player.dynamic.primary && player.dynamic.primary
										.player.chuchang
								},
								content: function() {
							...
							lib.skill._checkDcdShan = {
								...
								filter: function(event, player) {
									// 打出闪时
									return event.card.name === 'shan' && player.dynamic && (player
										.dynamic.primary && player.dynamic.primary.player
										.shizhounian || player.dynamic.deputy && player.dynamic
										.deputy.player.shizhounian)
								},
								content: function() {
									// 如果是双将, 只指定一个进行
									if (player.dynamic.primary && player.dynamic.primary.player
										.shizhounian) {
										skinSwitch.postMsgApi.action(player, player.dynamic.primary
											.player.shan || 'play3', player.dynamic.primary)
									} else {
										skinSwitch.postMsgApi.action(player, player.dynamic.deputy
											.player.shan || 'play3', player.dynamic.deputy)
									}
							...
							lib.skill._fix_yh = {
								...
								forced: true,
								filter: function(event, player) {
									return !(lib.config[skinSwitch.decadeKey.newDecadeStyle] ===
										"on")
								},
								content: function() {
							...
							let gameInit = function(character, character2, skill) {
								...
								if (!game.minskin && get.is.newLayout() && !info[4].contains(
									'minskin')) {
								...
								if (this.classList.contains('minskin') && this.node.name
									.querySelectorAll('br').length >= 4) {
								...
								if (info[4].contains('hiddenSkill') && !this.noclick) {
									...
									if (!this.node.name_seat && !_status.video) {
										this.node.name_seat = ui.create.div('.name.name_seat', get
											.verticalStr(get.translation(this.name)), this);
										this.node.name_seat.dataset.nature = get.groupnature(this
										.group);
									}
									this.sex = 'male';
									//this.group='unknown';
							...
							Player.init = function(character, character2, skill) {
								...
								let showDynamic = (this.dynamic || CUR_DYNAMIC < MAX_DYNAMIC) && duicfg
									.dynamicSkin;
								let y = false;
								if (showDynamic && _status.mode != null) {
									let skins;

									let avatars = this.doubleAvatar ? [character, character2] : [
										character
									];
									let dskins = decadeUI.dynamicSkin;
									let increased;
									...
									for (let i = 0; i < avatars.length; i++) {
										...
										if (lib.config[dynamicSkinKey]) {
											...
											if (ps) {
												...
												lib.config[dynamicSkinKey][avatars[i]] = Object.keys(
													skins)[0];
												game.saveConfig(dynamicSkinKey, lib.config[
													dynamicSkinKey]);
											}
										} else {
											lib.config[dynamicSkinKey] = {};
											lib.config[dynamicSkinKey][avatars[i]] = Object.keys(skins)[
												0];
										...
										if (skin.localePath) {
											...
											if (!skin.name.startsWith(skin.localePath + '/')) {
												...
												skin.background = skin.localePath + '/' + skin
													.background
											}
										}
										...
										if (!this.doubleAvatar && !isHide) {
											if (skin.background) {
												this.$dynamicWrap.style.backgroundImage = 'url("' + lib
													.assetURL + 'extension/十周年UI/assets/dynamic/' + skin
													.background + '")';
											} else {
												this.$dynamicWrap.style.backgroundImage = 'url("' + lib
													.assetURL + 'extension/皮肤切换/images/card/card.png")'
											}
										}
									...
									if ((this.doubleAvatar && (get.mode() === "guozhan")) ||
										hasHideWuJiang) {
							...
							Player.showCharacter = function(num, log) {
								let toShow = [];
								if ((num === 0 || num === 2) && this.isUnseen(0)) toShow.add(this
								.name1);
								if ((num === 1 || num === 2) && this.isUnseen(1)) toShow.add(this
								.name2);
							...
							Player.playDynamic = function(animation, deputy) {
								...
								if (!dynamic) {
									...
									skinSwitch.rendererOnMessage.addListener(this, 'logMessage',
										function(data) {
											console.log('dyWorker', data)
										})

								}
								let isCutBg = lib.config[skinSwitch.configKey.cugDynamicBg] && ui.arena
									.dataset.dynamicSkinOutcrop === 'on' && (ui.arena.dataset
										.newDecadeStyle === 'on')
							...
							// 先初步进行初始化
							if (!lib.config['extension_千幻聆音_enable'] || lib.config[
									'extension_千幻聆音_qhly_decadeCloseDynamic'] || !(lib.config
									.qhly_currentViewSkin === 'decade' || lib.config
						...
						if (!lib.config['extension_千幻聆音_enable'] || lib.config[
								'extension_千幻聆音_qhly_decadeCloseDynamic'] || !(lib.config
								.qhly_currentViewSkin === 'decade' || lib.config.qhly_currentViewSkin ===
								'shousha')) {
							retryOverride(20)
						}
					}
		...
		precontent: function() {
			window.skinSwitch = {
				...
				skinSwitchCheckYH: function(player, forces) {
					if (lib.config['extension_十周年UI_newDecadeStyle'] != "hahah") return;
						...
						for (let f of files) {
							game.readFile(skinSwitch.dcdPath + '/' + f, function(data) {
								game.writeFile(data, backDir, f, function() {
									console.log(`备份${f}成功`)
									skinSwitch.addProgress(progressBar, ++
										current, tasks)
				...
				modifyFileDui: function() {
					...
					if (confirm("会覆盖十周年原文件,请确认是否已经备份过原文件方便出错还原, 是否确认?")) {
						...
						cpWorkerFiles.forEach(cpWorkerFile => {
							game.readFile(skinSwitch.path + '/十周年UI/' + cpWorkerFile,
								function(data) {
									game.writeFile(data, skinSwitch.dcdPath,
										cpWorkerFile,
										function() {
											skinSwitch.addProgress(progressBar, ++
												current, tasks)
											if (current >= tasks) {
												game.saveConfig(skinSwitch.configKey
													.bakeup, true)
												setTimeout(() => {
													progressBG.style
														.opacity = "0";
													if (confirm(
															"导入备份十周年文件成功，点击确定将重启游戏"
															)) {
														progressBG.remove();
														game.reload();
													}
				...
				resetDynamicData: function() {
					if (!lib.config[skinSwitch.decadeKey.dynamicSkin]) return alert(
						"需要先打开十周年UI的动皮,再重置");
				...
				qhly_open_small: function(name, player, isPrimary) {
					...
					let backgroundBack = ui.create.div('.pfqh-qh-skinchange-background',
					background);
					...
					if (!skinSwitch.smallWindowAm) {
						...
						skinSwitch.smallWindowAm = new AnimationManager(lib.assetURL +
							'extension/十周年UI/assets/dynamic/', skinCanvas, 33321, {
								offscreen: false
							})
					...
					am.updateSpineAll({
						...
						dpr: Math.max(self.devicePixelRatio * (self.documentZoom ? self
							.documentZoom : 1), 1)
					})
					...
					let viewState = {
						...
						content: content,
						refresh: function() {
							this.content.style.width = Math.round(this.skinTotalWidth) +
								'px';
						...
						handleMouseMove: function(x, y) {
							if (this.isTouching) {
								...
								if (this.tempoffset > 0) {
									...
									.tempoffset) {
									this.tempoffset = -(this.skinTotalWidth - this
										.visibleWidth());
								}
								this.content.style.left = Math.round(this.tempoffset) +
								"px";
								return true;
							}
						},
						handleMouseUp: function(x, y) {
							if (this.isTouching) {
								...
								if (x && y) {
									...
									if (this.tempoffset > 0) {
										this.tempoffset = 0;
									} else if (this.skinTotalWidth - this.visibleWidth() < -
										this.tempoffset) {
										this.tempoffset = -(this.skinTotalWidth - this
											.visibleWidth());
									}
								}
								this.cancelClick = Math.abs(this.offset - this.tempoffset) >
									50;
								this.content.style.left = Math.round(this.tempoffset) +
								"px";
					...
					if (lib.config.touchscreen) {
						content.addEventListener('touchstart', function(event) {
							if (event.touches && event.touches.length) {
								viewState.handleMouseDown(event.touches[0].clientX, event
									.touches[0].clientY);
							}
						});
						...
						content.addEventListener('touchmove', function(event) {
							if (event.touches && event.touches.length)
								viewState.handleMouseMove(event.touches[0].clientX, event
									.touches[0].clientY);
						});
					} else {
						content.addEventListener('mousewheel', function(event) {
							...
							if (event.wheelDelta > 0) {
								viewState.handleMouseMove(event.clientX - 30, event
								.clientY);
					...
					keys.map(name => {
						skinInfoMap[name] = {
							staticImg: "url(" + skinSwitch.url +
								"/images/character/小杀.png)",
							dynamic: true,
						...
						let path = skinSwitch.dcdPath + '/assets/dynamic/' + foldPath +
							'/' + skins[name].skinName + '.jpg'
						// 如果该皮肤存在, 那么设置该皮肤为静态皮肤
						skinSwitch.qhly_checkFileExist(path, exists => {
							if (exists) {
								skinInfoMap[name].staticImg = 'url("' + lib
									.assetURL + path + '")'
							}
						})
					})
					...
					let playDynamic = (skinView, skinParams) => {
						...
						skinView.style.backgroundImage = 'url("' + lib.assetURL +
							'extension/十周年UI/assets/dynamic/' + sprite.background + '")'

						let dynamic = am.getAnimation(sprite.player.version)
						let beijingDynamic
						if (sprite.player && sprite.player.beijing != null) {
							beijingDynamic = am.getAnimation(sprite.player.beijing.version ||
								sprite.player.version)
						}

						let loadDaiJi = () => {
							if (!dynamic.hasSpine(sprite.name)) {
								dynamic.loadSpine(sprite.name, sprite.player.json ? 'json' :
									'skel', () => {
										// 加载后播放背景和待机
						...
						let loadBeiJingDaiJi = () => {
							if (!beijingDynamic.hasSpine(sprite.player.beijing.name)) {
								beijingDynamic.loadSpine(sprite.player.beijing.name, sprite
									.player.beijing.json ? 'json' : 'skel', () => {
										// 加载后播放背景和待机
										loadDaiJi()
									})
							} else {
						...
						let run = function(beijingNode) {
							...
							if (labels.includes(jinchangLabel)) {
								...
								if (t.player.action && t.player.action !== jinchangLabel) {
									t.skeleton.state.addAnimation(0, t.player.action, true,
										-0.01);
									...
									for (let defaultDaiJi of ['DaiJi', 'play']) {
										...
										if (da) {
											t.skeleton.state.addAnimation(0, da, true, -
												0.01);
											t.player.action = da
											t.action = da
										}
							...
							let setAnimation = () => {
								...
								for (let lab of ['TeShu', 'play2']) {
									if (labels.includes(lab) && lab !== daijiActioh) {
										t.skeleton.state.addAnimation(0, lab, false, 0)
											.listener = {
												complete: function(trackIndex) {
													t.skeleton.state.addAnimation(0,
														daijiActioh, true, 0)
													setAnimation()
												}
											}
						...
						let runBeijing = () => {
							...
							// 查找背景是否也有出场标签
							if (chuChangLabel) {
								node.skeleton.state.setAnimation(0, chuChangLabel, false,
								0);
								// 获取所有actions
								...
								for (let label of labels) {
									...
									for (let daijiName of ['DaiJi', 'BeiJing', 'play']) {
										if (daijiName.toLowerCase() === lowerLabel) {
											node.skeleton.state.addAnimation(0, label, true,
												-0.01);
											node.action = label
											break
										}
					...
					let setStaticSkin = () => {
						...
						if (player && player.dynamic) {
							...
							let obj = player.getElementsByClassName((bool1 ? 'primary' :
								'deputy') + "-avatar")[0]
							if (obj) {
								obj.style.opacity = 1
							}
					...
					let initSkinViews = () => {
						...
						viewState.skinTotalWidth = (viewState.skinPerWidth + viewState
							.skinGap) * skinKeys.length - viewState.skinGap;
						for (let i = 0; i < skinKeys.length; i++) {
							...
							skinView.style.left = Math.round((viewState.skinPerWidth + viewState
								.skinGap) * i) + "px";
							...
							skinView.belowText = ui.create.div('.pfqh-qh-skinchange-skin-text',
								skinView);
							if (i !== skinKeys.length - 1) {
								let border = ui.create.div('.pfqh-qh-skinchange-border',
									content);
								border.style.width = Math.round(viewState.skinGap) + "px";
								border.style.left = Math.round((viewState.skinPerWidth +
									viewState.skinGap) * i + viewState.skinPerWidth) + "px";
							}
							let skinSprite
							// 只有包含千幻聆音扩展才支持设置静皮
							if (skinInfo.dynamic) {
								...
								if (lib.config[skinSwitch.configKey.previewSkinsDynamic]) {
									skinView.style.backgroundImage = 'url("' + lib.assetURL +
										'extension/十周年UI/assets/dynamic/' + skinSprite
										.background + '")';
							...
							if (skinSwitch.qhly_hasExtension('千幻聆音')) {
								...
								if (skinInfo.dynamic) {
									...
									jingdong = ui.create.div(isDynamic ? '.pfqh-skin-dong' :
										'.pfqh-skin-jing', skinView)
									...
									jingdong.listen((e) => {
										if (!jingdong.isDynamic) {
											...
											skinSwitch.dynamic.selectSkinV3(skinKey,
												player, isPrimary)
											...
											if (!skinView.node) {
												skinView.style.backgroundImage =
													'url("' + lib.assetURL +
													'extension/十周年UI/assets/dynamic/' +
													skinSprite.background + '")';
												playDynamic(skinView, skinSprite)
											}
											...
											game.qhly_playQhlyAudio('qhly_voc_fanshu',
												null, true);

											// 设置当前皮肤的背景和语音, 调用千幻聆音
											// 恢复原来的静态背景
											if (!skinInfo.isDefault) {
												let file = game.qhly_getSkinFile(name,
													skin);
												skinView.qhly_origin_setBackgroundImage(
													file);
												game.qhly_setCurrentSkin(name, skin,
													function() {
														viewState.refreshSkins();
											...
											// 停止播放动画.
											if (skinView.node) {
												am.getAnimation(skinView.node.version)
													.stopSpine(skinView.node)
												if (skinView.node.beijingNode) {
													am.getAnimation(skinView.node
															.beijingNode.version)
														.stopSpine(skinView.node
															.beijingNode)
												}
								...
								(function(name, skin, view) {
									view.listen(function() {
										if (viewState.cancelClick) return;
										if (skin !== '__default_dynamic' && game
											.qhly_skinIs(name, skin)) return;
										game.qhly_playQhlyAudio('qhly_voc_fanshu',
											null, true);
								...
								if (skin) {
									if (!skinInfo.dynamic || !lib.config[skinSwitch.configKey
											.previewSkinsDynamic]) {
										if (skinInfo.isDefault) {
											skinView.style.backgroundImage = skinInfo.staticImg
										} else {
					...
					enlarge.listen(function() {
						exitListener();
						if (skinSwitch.qhly_hasExtension('千幻聆音')) {
							game.qhly_open_new(name, lib.config.qhly_doubledefaultpage ? lib
								.config.qhly_doubledefaultpage : 'skill', player);
						}
					})
				},
				selectSkinData: {
				...
				dynamic: {
					initSwitch: function(player, skins) {
						...
						skinDiv.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
							function() {
								skinSwitch.dynamic.skinDivShowOrHide()
							})
						var skinDiv2 = ui.create.div("#skinDiv2", skinDiv);
						skinDiv2.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
							function(e) {
								e.stopPropagation();
							})
						var skinBox = ui.create.div(".skinBox", skinDiv2);
						skinBox.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
							function(e) {
								e.stopPropagation();
							})
						...
						for (let i = 0; i < keys.length; i++) {
							...
							t.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
								function(e) {
									e.stopPropagation();
								})
							let path = skinSwitch.url + "/images/character/" + skinSwitch.dynamic
								.judgingRealName(name) + "/" + keys[i] + ".png";
							let img = document.createElement("img");
							let saveDynamic = lib.config[skinSwitch.configKey.dynamicSkin];
							if (saveDynamic) {
								var skin = saveDynamic[name];
								if (skin == keys[i]) {
									t.style.backgroundImage = "url(" + skinSwitch.url +
										"/images/base/skin_bg.png)";
									skinSwitch.selectSkinData.temp = t;
									skinSwitch.selectSkinData.value = keys[i];
								} else t.style.backgroundImage = "url(" + skinSwitch.url +
									"/images/base/skin_not_bg.png)";
							}

							img.alt = keys[i];
							img.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
								function(e) {
									e.stopPropagation();
						...
						if (skinInfos) {
							...
							for (let i = 0; i < keys.length; i++) {
								...
								if (curIndex <= 2) {
									...
									if (i + 2 < curIndex || i > curIndex) skinAvatar.style.display =
										'none'
								}
								skinImgDiv.setAttribute('skinName', k)
								skinImgDiv.addEventListener(addLisName, (e) => {
									e.stopPropagation()
									skinSwitch.dynamic.selectSkinV2(e.target.getAttribute(
										'skinName'), e.target)
								})
								...
								if (selectName === k) {
									...
									// 并且设置选择的是当前皮肤
									let selInfo = skinSwitch.dynamic.playerTempSkinInfo[player
										.pfqhId]
									if (isPrimary) {
										selInfo.primary = {
											temp: skinImgDiv,
								...
								if (skinSwitch.qhly_hasExtension('千幻聆音')) {
									...
									skinSwitch.checkFilesExistAndReturnOne([filename + '.jpg',
										filename + '.png', skinSwitch.dcdPath +
										'/assets/dynamic/' + foldPath + '/' + skinInfo
										.skinName + '.jpg'
									], (path) => {
										if (path) {
											skinImgDiv.style.backgroundImage = "url(" + lib
												.assetURL + path + ")"
										} else {
											skinImgDiv.style.backgroundImage = "url(" +
												skinSwitch.url + "/images/character/小杀.png)"
										}

									})
								} else {
									skinImgDiv.style.backgroundImage = "url(" + skinSwitch.url +
										"/images/character/小杀.png)"
								}
							}
							if (keys.length < 3) {
					...
					initSwitchV2: function() {
						...
						if (Object.keys(this.dynamicSkinInfo).length > 0) {
                            ...
                            `
							document.getElementById('skinCharacter').addEventListener(addLisName,
								e => {
									e.stopPropagation()
								})
							...
							let changeDynamicSkinsByIdx = (idx) => {
								...
								if (this.dynamicSkinInfo[idx].primary) {
									initPlayerAvatarDynamic(this.dynamicSkinInfo[idx].player,
										true)
							...
							playerSkinSelect.onchange = (e) => {
								let idx = playerSkinSelect.options[playerSkinSelect
									.selectedIndex].value
							...
							btn.addEventListener(addLisName, e => {
								let curSelect = this.dynamicSkinInfo[this.playerTempSkinInfo
									.currentWatchId]
								...
								if (!curSelect.zhuFuFlag) {
									...
									text1.classList.remove('active');
									text2.classList.add('active');
									initPlayerAvatarDynamic(curSelect.player, curSelect
										.zhuFuFlag)
								} else {
									btn.classList.add('left');
									btn.classList.remove('right');
							...
							document.getElementById('dynamicRightArrow').addEventListener(
								addLisName, (e) => {
									let skins = document.getElementById('pfqhSkins').children
									// 获取当前是主将还是副将
									let flag = this.dynamicSkinInfo[this.playerTempSkinInfo
										.currentWatchId].zhuFuFlag
									let watchId = this.playerTempSkinInfo.currentWatchId
									let avatar = flag ? this.playerTempSkinInfo[watchId]
										.primary : this.playerTempSkinInfo[watchId].deputy
									...
									avatar.curIndex++
									if (skins.length <= avatar.curIndex + 3) e.target.classList
										.add('hidden')
									document.getElementById('dynamicLeftArrow').classList
										.remove('hidden')
								})

							document.getElementById('dynamicLeftArrow').addEventListener(addLisName,
								(e) => {
									let skins = document.getElementById('pfqhSkins').children
									let flag = this.dynamicSkinInfo[this.playerTempSkinInfo
										.currentWatchId].zhuFuFlag
									...
									document.getElementById('dynamicRightArrow').classList
										.remove('hidden')
								})
					...
					selectSkinV2: function(skinName, target) {
						...
						if (avatarSkins && avatarInfo && avatarInfo.value !== skinName) {
							...
							if (game.qhly_setCurrentSkin) {
								...
								game.qhly_setCurrentSkin(isPrimary ? player.name1 : player.name2,
									skin.skinName + '.jpg', () => {
										...
										// 默认用.jpg结尾,
										lib.config.qhly_skinset.djtoggle[namex][skin.skinName +
											'.jpg'
										] = false
									}, true)
							}
							...
							if (!lib.config[skinSwitch.configKey.dynamicSkin]) lib.config[skinSwitch
								.configKey.dynamicSkin] = {};
							if (lib.config[skinSwitch.configKey.dynamicSkin]) {
					...
					selectSkin: function(e) {
						...
						if (temp !== e) {
							if (skinSwitch.dynamic.selectSkin.cd) {
								...
								let player = game.me;
								temp.style.backgroundImage = "url(" + skinSwitch.url +
									"/images/base/skin_not_bg.png)";
								skinSwitch.selectSkinData.value = e.alt;
								e.style.backgroundImage = "url(" + skinSwitch.url +
									"/images/base/skin_bg.png)";
								skinSwitch.selectSkinData.temp = e;
								...
								if (skin.background) {
									player.$dynamicWrap.style.backgroundImage = 'url("' + lib
										.assetURL + 'extension/十周年UI/assets/dynamic/' + skin
										.background + '")';
								} else {
									player.$dynamicWrap.style.backgroundImage = 'url("' + lib
					...
					setBackground: function(avatar, player) {
						...
						if (obj.style.backgroundImage == null) {
							...
							if (skin.qhly_hasExtension('千幻聆音')) {
								...
								obj.style.backgroundImage = 'url("' + lib.assetURL +
									'extension/十周年UI/assets/dynamic/' + skin.background + '")';
							}
						}
						...
						if (player.$dynamicWrap && skin.player && skin.player.background) {
							player.$dynamicWrap.style.backgroundImage = 'url("' + lib.assetURL +
								'extension/十周年UI/assets/dynamic/' + skin.player.background + '")';
						}
					...
					startPlay2Random: function(player) {
						...
						let checkCanPlay2 = (isPrimary) => {
							if (!player.dynamic) return false
							let avatar = isPrimary ? player.dynamic.primary : player.dynamic
								.deputy
							if (avatar) {
						...
						if (!player.playPrimaryTimer) {
							if (checkCanPlay2(true)) {
								...
								let randomInterval = function(timer) {
									...
									// 只有非攻击状态才播放play2
									if ((!player.lastPlayTime || (new Date().getTime() - player
											.lastPlayTime) > 10000) && !player.GongJi) {
						...
						if (!player.playDeputyTimer) {
							if (checkCanPlay2(false)) {
								...
								let randomInterval = function() {
									...
									if ((!player.lastPlayTime || (new Date().getTime() - player
											.lastPlayTime) > 8000) && !player.GongJi) {
					...
					transformDst: (player, isPrimary, dstInfo, extraParams = {
						...
						if (dstInfo.name == null || dstInfo.name === avatar.name) {
							...
							if (dstInfo.skin) {
								skinSwitch.postMsgApi.changeSkelSkin(player, dstInfo.skin,
									isPrimary)
							}
						} else {
							...
							if (huanfuEffect) {
								...
								huanfuEff.name = '../../../皮肤切换/effects/transform/' + huanfuEffect
									.name
							}
				...
				postMsgApi: {
					_onchangeDynamicWindow: function(player, res) {
						...
						skinSwitch.rendererOnMessage.addListener(player, 'chukuangFirst', function(
							data) {
						...
						skinSwitch.rendererOnMessage.addListener(player, 'canvasRecover', function(
							data) {
							if (lib.config['extension_十周年UI_newDecadeStyle'] === "on") {
						...
						skinSwitch.rendererOnMessage.addListener(player, 'chukuangSecond', function(
							data) {
							// 这里表示动画已经准备好了, 可以显示
							setTimeout(() => {
							...
							if (res.dynamic.localePath && playName.startsWith(res.dynamic
									.localePath)) {
								playName = playName.substr(res.dynamic.localePath.length +
									1, playName.length)
							}
							game.playAudio("..", "extension", "皮肤切换/audio/effect",
								playName + ".mp3");
						})
					},
					...
					actionTeShu: function(player) {
						...
						if (r) {
							...
							skinSwitch.rendererOnMessage.addListener(player, 'teshuChuKuang',
								function(data) {
									if (data.chukuang) {
					...
					show: function(player, skinId) {
						if (!(player.dynamic && (player.dynamic.primary || player.dynamic
							.deputy))) {
							skinSwitchMessage.show({
								type: 'error',
								text: '只有当前角色是动皮时才可以编辑动皮参数',
					...
					startPlay: function(player, data) {
						if (!player.dynamic) return
						skinSwitch.rendererOnMessage.addListener(player, 'logMessage', function(
							data) {
							console.log('dyWorker', data)
						})
				...
				chukuangWorkerApi: {
					create: function() {
						...
						canvas.style =
							`position: fixed; left: 0px; top: 0px; pointer-events:none; width:100%;height:100%;`
						...
						skinSwitch.chukuangWorker.postMessage({
							...
							dpr: Math.max(window.devicePixelRatio * (window.documentZoom ?
								window.documentZoom : 1), 1),
					...
					chukuangAction: function(player, action, extraParams) {
						...
						skinSwitch.chukuangWorker.postMessage({
							message: 'isChuKuang',
							id: dynamic.id,
							primarySkinId: (!player.isUnseen || !player.isUnseen(0)) &&
								dynamic.primary && dynamic.primary.id,
							deputySkinId: (!player.isUnseen || !player.isUnseen(1)) &&
								dynamic.deputy && dynamic.deputy.id,
							action: action,
				...
				chukuangWorkerOnMessage: {
					...
					chukuangStart: function(data) {
						...
						if (_status.currentPhase !== player && data.action === 'TeShu' && !avatar
							.player.shizhounian) {
							return
						}
						player.dynamic.renderer.postMessage({
						...
						skinSwitch.rendererOnMessage.addListener(player, 'hideAllNodeEnd',
						function() {
							let pp = skinSwitch.getCoordinate(player, true)
							let me = player === game.me
							skinSwitch.chukuangWorker.postMessage({
								...
								action: data.action,
								me: me,
								direction: me ? false : skinSwitch.getDirection(
									player),
								player: pp,
							})
							// 标记为出框状态
							...
							if (data.action === 'GongJi' || data.action === 'TeShu') {
								...
								let path = 'extension/十周年UI/assets/dynamic/' + playName +
									'.mp3'
								skinSwitch.qhly_checkFileExist(path, exists => {
									if (exists) {
										game.playAudio("..", path)
						...
						if (data.action === 'GongJi') {
							...
							if (dynamic.primary) {
								...
								if (playerP.gongji && playerP.gongji.ck === false) {
									return skinSwitch.postMsgApi.action(player, playerP.gongji
										.action, dynamic.primary)
								}
							}
							if (dynamic.deputy) {
				...
				cleanupAfterStopDynamic: function(player, primary, deputy) {
					...
					let clearAudioMap = (name) => {
						if (skinSwitch.avatarAudioSkinMap) {
							...
							console.log('skinSwitch.audioMap', skinSwitch.audioMap,
								'skinSwitch.avatarAudioSkinMap', skinSwitch
								.avatarAudioSkinMap)
						}
					}
				...
				chukuangPlayerInit: function(player, isPrimary, playParams) {
					...
					if (!(isPlayer || player.classList.contains('qh-shousha-big-avatar') || player
							.getElementsByClassName('qhdynamic-decade-big-wrap').length || player
							.getElementsByClassName('qhdynamic-big-wrap').length)) {
						return
					}
					if (!isPlayer) {
					...
					let initPlayerAudio = () => {
						...
						let id = player.dynamic.id
						let skinId = isPrimary ? player.dynamic.primary.id : player.dynamic
							.deputy.id
						...
						if (playParams.audio && isPlayer) {
							...
							if (skillPath) {
								...
								game.getFileList(path, function(folds, files) {
									...
									for (let file of files) {
										...
										if (file === name) {
											key = 'die/' + file
											skinSwitch.audioMap[key] = '../' + path +
												'/' + file;
										} else if (file === 'victory' || file ===
											'win') {
											key = 'effect/' + id + '/' + skinId + '/' +
												'victory'
											skinSwitch.audioMap[key] = '../' + path +
												'/' + file;
										} else {
							...
							if (cardPath) {
								...
								game.getFileList(path, function(folds, files) {
									for (let file of files) {
										...
										// 储存动皮相关的id和角色名字
										let id = player.dynamic.id
										let skinId = isPrimary ? player.dynamic.primary
											.id : player.dynamic.deputy.id
										let key = 'card/' + id + '/' + skinId + '/' +
											file
										skinSwitch.audioMap[key] = '../' + path + '/' +
											file
										skinSwitch.avatarAudioSkinMap[name][key] = null
							...
							if (!this._initAudio) {
								...
								game.playAudio = function() {
									...
									if (string.startsWith('/card') && replaces.length ===
										4) {
										let cardName = replaces[3]
										// 检索待播放队列是否进行替换
										if (skinSwitch.audioPlayQueue) {
											for (let i = 0; i < skinSwitch.audioPlayQueue
												.length; i++) {
												if (new Date().getTime() - skinSwitch
													.audioPlayQueue[i].time > 2000) {
													// 删除超时未播放的或不存在的语音
												...
												if (au.card === cardName) {
													replace = 'card/' + au.id + '/' + au
														.skinId + '/' + cardEn2Cn[cardName]
													if (skinSwitch.audioMap[replace]) {
														skinSwitch.audioPlayQueue.splice(i,
															1)
														break
													} else {
														replace = 'card/' + au.id + '/' + au
									...
									} else if (string.startsWith('/skill') && replaces
										.length === 3) {
										replace = string.slice(1)
									} else if (string.startsWith('/die') && replaces
										.length === 3) {
										// 检索待播放队列是否进行替换
										replace = string.slice(1)
									} else if (string.startsWith('/effect/win')) {
										replace = 'effect/' + id + '/' + skinId + '/' +
											'victory'
									}
									...
									if (replace.length) {
										...
										if (rp) {
											...
											return skinSwitch.pfqh_originPlayAudio.apply(
												this, args);
										}
									}
									return skinSwitch.pfqh_originPlayAudio.apply(this,
										arguments);
								};
								...
								game.playSkillAudio = function(name, index) {
									...
									return skinSwitch.qfqh_originPlaySkillAudio.apply(this,
										arguments);
								};
					...
					let checkChangeSkin = () => {
						...
						let path = skinSwitch.dcdPath + '/assets/dynamic/' + foldPath + '/' +
							playParams.skinName + '.jpg'
						// 如果该皮肤存在, 那么设置该皮肤为静态皮肤
						skinSwitch.qhly_checkFileExist(path, exists => {
							if (exists) {
								let avatar = player.getElementsByClassName((isPrimary ?
									'primary' : 'deputy') + '-avatar')
								if (avatar.length) {
									avatar[0].style.backgroundImage = 'url("' + lib
										.assetURL + path + '")'
								}
							}
						})
					...
					if (!isPrimary && player.dynamic.deputy) {
						skinSwitch.chukuangWorkerApi.preLoad(player.dynamic.id, player.dynamic
							.deputy.id, playParams)
					} else if (isPrimary && player.dynamic.primary) {
						skinSwitch.chukuangWorkerApi.preLoad(player.dynamic.id, player.dynamic
							.primary.id, playParams)
					}
				},
				...
				previewDynamic: function() {
					...
					previewWindow.style =
						`background: rgb(60,60,60);z-index: 3000;position: fixed; width: 100%; height: 100%;`
...
<!--                        <span>Debug:</span><input type="checkbox" id="debug">-->
					...
					let dpr = Math.max(window.devicePixelRatio * (window.documentZoom ? window
						.documentZoom : 1), 1)
					...
					if (!currentPath) {
						game.saveConfig(skinSwitch.configKey.lastPreviewPath,
							'extension/皮肤切换/assets')
					...
					let contentModal = document.getElementById('unique-id').getElementsByClassName(
						'light-modal-body')[0]
					...
					const returnLastDir = function(e) {
						...
						let lastDirName
						if (currentPath) {
							lastDirName = currentPath.substring(currentPath.lastIndexOf('/') +
								1, currentPath.length)
						}
						currentPath = currentPath === '' ? '' : currentPath.substring(0,
					...
					// 同返回上一级
					document.getElementById('btnReturnLastDir').addEventListener(clickName,
						function(e) {
							returnLastDir()
						})
					// 收缩文件夹
					document.getElementById('btnShouSuoDir').addEventListener(clickName, function(
					e) {
					...
					let sortDirs = (sorts) => {
						...
						if (sorts == null || sorts === 0) {
							arr.sort(function(a, b) {
								return a.getAttribute('fold').localeCompare(b
									.getAttribute('fold'))
							})

						} else if (sorts === 1) {
							arr.sort(function(a, b) {
								return b.getAttribute('fold').localeCompare(a
									.getAttribute('fold'))
							})
						}
					...
					document.getElementById('btnSortDirFiles').addEventListener(clickName, function(
						e) {
					...
					let initFoldsInfo = (lastDirName) => {
						...
						pfqhUtils.getFoldsFiles(currentPath, function(file, path) {
							...
							// 排序一下节点
							let _sorts = document.getElementById('btnSortDirFiles')
								._sorts
							if (_sorts != null) {
								sortDirs(_sorts)
							}
							for (let i = 1; i < foldsEle.children.length; i++) {
								...
								if (lastDirName && lastDirName === foldName) {
									...
									foldsEle.children[i].classList.remove(
										'previewSelect')
								}
							}
							...
							for (let i = 0; i < retFiles.length; i++) {
								...
								div.addEventListener(clickName, function(e) {
									...
									// 可能需要更新一下滚动条以及索引
									for (let idx = 0; idx < this.parentNode
										.children.length; idx++) {
										if (this === this.parentNode.children[
												idx]) {
											// 清理图集相关的信息
											currentFoldInfo.curFileIndex = idx
											if (window.location.hash ===
												'#unique-id') {
												let scrollTop = filesEle
						...
						pfqhUtils.getSpineFileVersion(fullPath, function(version) {
							...
							if (!isUpdate) {
								dy.update({
									...
									dpr: Math.max(window.devicePixelRatio * (
										window.documentZoom ? window
										.documentZoom : 1), 1),
								})
							}

							let play = () => {
								if (skinSwitch.nodePreviewedInfo[filename] ===
									false) return
								// 播放下一个之前, 保存上一个的node的数据
								if (currentNode) {
									skinSwitch.nodePreviewedInfo[currentNode
									.name] = {
										x: [0, x / canvas.width],
										y: [0, y / canvas.height],
										...
										action: currentNode.skeleton.state
											.tracks[0].animation.name,
									}
								}
								...
								if (filename in skinSwitch.nodePreviewedInfo) {
									playInfo = Object.assign(playInfo, skinSwitch
										.nodePreviewedInfo[filename])
									isPreviewed = true
								}
								...
								let activeAnimation = state.tracks[0].animation
								.name;

								let checkHasDaiJi = (act) => {
									// 自动寻找待机标签
									const idleAction = ['idle', 'daiji', 'play',
										'animation'
									]
									let res = false
									for (let idle of idleAction) {
										if (act.toLowerCase().indexOf(idle) !==
											-1) {
											res = true
											break
										}
								...
								if (!isPreviewed && !checkHasDaiJi(
									activeAnimation)) {
									for (let i = 1; i < node.skeleton.data
										.animations.length; i++) {
										let name = node.skeleton.data.animations[i]
											.name
										if (checkHasDaiJi(name)) {
											node.skeleton.state.setAnimation(0,
												name, true);
											node.skeleton.setToSetupPose();
											// 重新计算bounds
											node.skeleton.updateWorldTransform();
											node.skeleton.bounds = {
												offset: new dy.spineLib
												.Vector2(),
												size: new dy.spineLib.Vector2()
											};
											node.skeleton.getBounds(node.skeleton
												.bounds.offset, node.skeleton
												.bounds.size, []);
								...
								let autoFit = (t) => {
									...
									if (!Number.isFinite(t.skeleton.bounds
											.offset.x)) {
										initCurrentNodeInfo()
										return
									}

									let centerX = bounds.offset.x + bounds.size
										.x / 2;
									let centerY = bounds.offset.y + bounds.size
										.y / 2;
									...
									if (tempScale < 1) {
										t.scale = tempScale * height / width *
											1.2
									} else {
										t.scale = tempScale * height / width /
											1.2
									...
									if (picPreviewBg.classList.contains(
											'hidden')) {
										yy -= 0.07
									}

									if (!document.getElementById(
											'previewDetailInfo').classList
										.contains('hidden')) {
										xx -= 0.1;
									}
								...
								document.getElementById('curVersionText')
									.innerText = `当前版本: ${version}`
								// 切换当前的骨骼
							...
							if (dy.hasSpine(filename)) {
								...
								dy.loadSpine(filename, ext, () => {
									...
									skinSwitch.nodePreviewedInfo[filename] =
										false
								})
							}
					...
					let timeSlider {
						...
						timeSlider.addEventListener('input', debounce(function(e) {
							text.innerHTML =
								`进度: ${Number(timeSlider.value).toFixed(2)}`
							...
							if (currentNode) {
								...
								entry.trackTime = Number(timeSlider.value) * entry
									.animationEnd
							}
					...
					let closePreviewWindow = document.getElementById('closePreviewWindow')
					closePreviewWindow.parentNode.insertBefore(picPreviewModeBtn,
						closePreviewWindow)
					...
					const previewOperateSpeedCtrl = document.getElementById(
						'previewOperateSpeedCtrl')

					// 详情
					const previewDetailInfo = document.getElementById('previewDetailInfo')
					const previewInfoHideMenu = document.getElementById(
						'previewInfoHideMenu') // 隐藏按钮

					const operateBtn = {
						left: document.getElementById('previewOperateLeft'),
						right: document.getElementById('previewOperateRight'),
						switchAction: document.getElementById(
						'previewOperateQieHuan'), // 快速切换标签
					...
					document.getElementById('opFoldTree').listen(() => {
						if (window.location.hash !== '#unique-id') {
							...
							setTimeout(() => {
								if (lastSelFile) {
									for (let idx = 0; idx < filesEle.children
										.length; idx++) {
										if (lastSelFile === filesEle.children[
											idx]) {
											if (idx > 5) {
												filesEle.parentNode.scrollTop = 40 *
													(idx - 5)
											}
											break
										}
					...
					picPreviewModeBtn.addEventListener(lib.config.touchscreen ? 'touchend' :
						'click',
						function() {
							// 隐藏原来的按钮, 进入图集模式
					...
					closeImgPreview.addEventListener(lib.config.touchscreen ? 'touchend' : 'click',
						function() {
					...
					operateBtn.left.listen((e) => {
						...
						if (len === 0) return
						if (currentFoldInfo.curFileIndex === -1) currentFoldInfo
							.curFileIndex = 0
						else {
							if (len === 1) return
							if (currentFoldInfo.curFileIndex === 0) currentFoldInfo
								.curFileIndex = len - 1
							else currentFoldInfo.curFileIndex--
						}
						currentFoldInfo.curFiles[currentFoldInfo.curFileIndex]
							.dispatchEvent(new CustomEvent(lib.config.touchscreen ?
								'touchend' : 'click'))
						// 关闭标签预览页
						closeModalFunc()
					})
					...
					operateBtn.right.listen((e) => {
						...
						if (currentFoldInfo.curFileIndex === -1) currentFoldInfo
							...
							if (len === 1) return
							if (currentFoldInfo.curFileIndex === len - 1) currentFoldInfo
								.curFileIndex = 0
							else currentFoldInfo.curFileIndex++
						}
						currentFoldInfo.curFiles[currentFoldInfo.curFileIndex]
					...
					operateBtn.alpha.listen(function(e) {
						let premultipliedAlpha = document.getElementById(
							'premultipliedAlpha')
					...
					operateBtn.speed.listen(function(e) {
						...
						if (currentNode) {
							...
							progressData.progressBarWrap.style.transform =
								`scaleY(${percent})`
							progressData.progressBarThumb.style.transform =
								`translateY(-${percent*78}px)`
						}
						closeModalFunc()
					...
					operateBtn.skin.listen(function(e) {
						...
						if (!previewImgModal.classList.contains('hidden')) {
							if (document.getElementById('previewModalTitle').innerText ===
								'骨骼皮肤') {
								closeModalFunc()
								return
							}
					...
					const initCurrentNodeInfo = (index) => {
						...
						if (currentNode) {
							if (index == null) {
								...
								contentNodes[1].innerText = document.getElementById(
									'curVersionText').innerText
								contentNodes[2].innerText = currentAni.duration.toFixed(1)
								contentNodes[3].innerText =
									`标签: ${currentAni.name}, 皮肤: ${currentNode.skeleton.skin.name}`
								let speed
								...
								contentNodes[4].innerText =
									`大小: ${Number(currentNode.scale).toFixed(2)}, 速度: ${speed}`
							} else {
								switch (index) {
									case 0:
										contentNodes[0].innerText = lastSelFile.getAttribute(
											'path')
										break
									case 1:
										contentNodes[1].innerText = document.getElementById(
										...
										break
									case 2:
										contentNodes[2].innerText = currentAni.duration.toFixed(
											1)
										break
									case 3:
										contentNodes[3].innerText =
					...
					let progressData = {
						...
						progressBarWrap: dataProgressBar.getElementsByClassName(
							'bui-bar-normal')[0],
						progressBarThumb: dataProgressBar.getElementsByClassName('bui-thumb')[
							0],

					}
					dataProgressBar.addEventListener(lib.config.touchscreen ? 'touchstart' :
						'mousedown',
						function(e) {
							e.stopPropagation()
					...
					picPreviewBg.addEventListener(lib.config.touchscreen ? 'touchstart' :
						'mousedown',
						function(e) {
					...
					picPreviewBg.addEventListener(lib.config.touchscreen ? 'touchmove' :
						'mousemove',
						function(e) {
					...
					picPreviewBg.addEventListener(lib.config.touchscreen ? 'touchend' : 'mouseup',
						function(e) {
							if (progressData.isDown) {
								initCurrentNodeInfo(4)
							}
					...
					picPreviewBg.addEventListener(lib.config.touchscreen ? 'touchcancel' :
						'mouseleave',
						function(e) {
							progressData.isDown = false
						})
					...
					function mousemoveEvent(event) {
						if (event) {
							if (event.touches && event.touches.length) {
								picPreviewBg._mousemove(event.touches[0].clientX, event.touches[0]
									.clientY);
					...
					function mousedownEvent(event) {
						if (event) {
							...
							if (event.touches && event.touches.length) picPreviewBg._mousedown(event
								.touches[0].clientX, event.touches[0].clientY);
					...
					const initItemsModal = (itemList, selectItem, func) => {
						...
						itemList.forEach(item => {
							...
							it.innerHTML =
								`<span class="actionItemTag"><i class="iconfont icon-collect actionItemTagIcon" ></i>${item}</span>`
							modalItemsList.appendChild(it)
							it.setAttribute('value', item)

							if (selectItem === item) {
								it.getElementsByTagName('span')[0].classList.add(
									'selectItemTag')
							}
							it.listen(function(e) {
								skinSwitch.refreshDomList(
									Array.from(modalItemsList.children).map(
										v => v.getElementsByTagName('span')[
											0]),
									'selectItemTag',
					...
					document.getElementById('closePreviewWindow').addEventListener(lib.config
						.touchscreen ? 'touchend' : 'click',
						function() {
							// 删除自己当前节点即可
							let self = background
					...
					function init() {
						...
						let setupAnimationUI = () => {
							...
							for (let i = 0; i < skeleton.data.animations.length; i++) {
								...
								if (name === activeAnimation) {
									...
									document.getElementById('aniTime').innerText = Number(
										skeleton.data.animations[i].duration).toFixed(1)
								}
							...
							animationList.onchange = function() {
								let skeleton = activeSkeleton
								let state = skeleton.state;
								let animationName = animationList.options[animationList
									.selectedIndex].text
								skeleton.setToSetupPose();
								state.setAnimation(0, animationName, true);
								let ani = skeleton.data.findAnimation(animationName)
								document.getElementById('aniTime').innerText = Number(ani
									.duration).toFixed(1)
							}
						}
						...
						currentNode.premultipliedAlpha = document.getElementById(
							'premultipliedAlpha').checked
				...
				cangZhenGe: function() {
					...
					const loadAnimations = () => {
						...
						app.loader
							.add("gongxihuode_biankuang", skinSwitch.url +
								"images/cangZhenGe/spineAni/gongxihuode/gongxihuode_biankuang.skel"
								)
							.add("aar_cangbaoge", skinSwitch.url +
								"images/cangZhenGe/spineAni/aar_cangbaoge.skel")
							.add('item_board', skinSwitch.url +
								'images/cangZhenGe/game_hist_headbg.png')
					...
					function drawItem(itemInfo, count, resource, hasSpine) {
						...
						// 边框
						const board = new PIXI.Sprite.from(skinSwitch.url +
							'images/cangZhenGe/game_hist_headbg.png')
						...
						if (itemInfo.type === 'wujiang') {
							item = new PIXI.Sprite.from(skinSwitch.url +
								`images/cangZhenGe/wujiang/${itemInfo.id}.png`);
						} else {
							item = new PIXI.Sprite.from(skinSwitch.url +
								`images/cangZhenGe/items/${itemInfo.id}.png`);
						}
						...
						setTimeout(() => {
							...
							board.visible = true
							if (hasSpine) {
								let chouzhong = new PIXI.spine.Spine(resource
									.gongxihuode_daojuchuxian.spineData)
							...
							if (itemInfo.gaoji) {
								// 如果是高级道具, 添加边框特效
								let gaojidaoju = new PIXI.spine.Spine(resource
									.gongxihuode_gaojidaoju.spineData);
					...
					function onAssetsLoaded(loader, resource) {
						...
						// 接着加载第二批资源
						app.loader
							.add("chouzhong", skinSwitch.url +
								"images/cangZhenGe/spineAni/Ss_M_WWJ_chouzhong.skel")
							.add("gongxihuode_daojuchuxian", skinSwitch.url +
								"images/cangZhenGe/spineAni/gongxihuode/gongxihuode_daojuchuxian.skel"
								)
							.add("gongxihuode_gaojidaoju", skinSwitch.url +
								"images/cangZhenGe/spineAni/gongxihuode/gongxihuode_gaojidaoju.skel"
								)
							.add("gongxihuode_lizi", skinSwitch.url +
								"images/cangZhenGe/spineAni/gongxihuode/gongxihuode_lizi.skel")
							.add('back', skinSwitch.url + 'images/cangZhenGe/dialog2.png')
							.add('rr_yuan_pan', skinSwitch.url +
								'images/cangZhenGe/rewardresult/rr_yuan_pan.png')
							.add('title', skinSwitch.url +
								'images/cangZhenGe/rewardresult/rr_title.png')
					}
					...
					openAllTip.innerHTML =
						'<span style=\'color:#DEB887; text-shadow:0 0 1px black;font-weight:600;font-family:shousha\'>每次最多开50个</span>'
					...
					boxBugTip.innerHTML =
						"<span style='color:red; text-shadow:0 0 0.5px gray,0 0 0.5px gray;font-weight:500;font-family:shousha'>2023年七夕活动首发，后续请关注每周末限时活动</span>";
					// 各个盒子部分
					...
					for (let i = 0; i < boxSettings.length; i++) {
						...
						boxDiv.listen(function() {
							...
							boxBugTip.getElementsByTagName('span')[0].innerText = boxInfo
								.tip
					...
					const xishiLabelText = ui.create.div('.xishizhenbao-label-text',
						xishi) // 稀释珍宝标识
					const xishiRareLabel = ui.create.div('.xishizhenbao-rare-label', xishi) // 史诗标识

					function setCurrentBoxUi(box) {
						xishi.style.backgroundImage = 'url("' + skinSwitch.url +
							`images/cangZhenGe/bskin/${box.xishizhenbao.id}.jpg")`
					...
					function bindDragEvent(divBg, scrollView) {
						...
						let mousemoveEvent = function(e) {
							...
							if (deltaY === 0 || top >= contentHeight && deltaY < 0 || deltaY >
								0 && top <= 0) {
								return
							} else {
								top -= deltaY
					...
					function openRewardResult(counts) {
									...
									if (randomR < weight_steps[j]) {
										...
										for (let r of result) {
											if (r.id === currenBox.items[j].id) {
												r.count = (r.count || 1) + (currenBox.items[j]
													.count || 1)
												isExist = true
												...
												if (currenBox.items[j].weight < (r.weight ||
													1000)) {
						...
						for (let r of results) {
							let v = totalRewards.items[r.id] || {
								id: r.id,
								name: r.name,
								count: 0,
								weight: r.weight || 3000
							}
						...
						for (let i = 0; i < results.length; i++) {
							...
							const _drawItem = (i) => {
								let rewardItem = drawItem(itemInfo, itemInfo.count || 1,
									resource, i < 12)

								if (results.length <= 6) {

									if (results.length <= 4) {
										let startX = (scrollBoxWidth - results.length *
											boardWidth * 1.2) / 2
										rewardItem.x = startX * 1.7 + (boardWidth * 1.1) * (i %
											results.length);
									} else if (results.length === 5) {
										let startX = (scrollBoxWidth - results.length *
											boardWidth * 1.1) / 2
										rewardItem.x = startX * 1.5 + (boardWidth * 1.1) * (i %
											results.length);
									} else {
										let startX = (scrollBoxWidth - 6 * boardWidth) / 2
										rewardItem.x = startX * 1.5 + (boardWidth * 1.05) * (i %
											6);
									}
									rewardItem.y = 56
						...
						// set the position
						setTimeout(() => {
							const xingxingAni = new PIXI.spine.Spine(resource
								.gongxihuode_lizi.spineData);
					...
					openAll.listen(function() {
						...
						if (boxbeijing) {
							...
							game.playAudio(
								'../extension/皮肤切换/images/cangZhenGe/mp3/guo.mp3')
							let lis = {
					...
					function drawPreviewItem(itemInfo) {
						...
						// 边框
						const board = new PIXI.Sprite.from(skinSwitch.url +
							'images/cangZhenGe/preview_reward.png')
						rewardItem.addChild(board)
						board.visible = true
						...
						if (itemInfo.gaoji) {
							// 如果是高级道具, 添加边框特效
							let gaojidaoju = new PIXI.spine.Spine(resource.gongxihuode_biankuang
								.spineData);
							rewardItem.addChild(gaojidaoju)
							setDefaultAni(gaojidaoju, true)
							let localPos = gaojidaoju.getLocalBounds()
				...
				openEventBindWindow: function(player, isPrimary) {
					...
					const transformBtn = ui.create.div('.transformBtn .eventBindButton .success',
						toolItems)
					const playEffectBtn = ui.create.div('.playEffectBtn .eventBindButton .success',
						toolItems)
					const previewBtn = ui.create.div('.previewBtn .eventBindButton .success',
						toolItems)
					...
					const contentHeaders = ui.create.div('.contentHeaders',
						transformContent) // 内容区域的选项区域
					...
					if (!skinSwitch.animationManager) {
						skinSwitch.animationManager = new AnimationManager(lib.assetURL +
							'extension/十周年UI/assets/animation/', dcdAnim.canvas, 988888, {
								offscreen: false
							})
					}
					...
					const initDiffSkelInfo = () => {
						...
						for (let i = 0; i < 2; i++) {
							...
							for (let t of letters) {
								...
								span.innerText = t
								span.addEventListener(lib.config.touchscreen ? 'touchend' :
									'click',
									function() {
						...
						const initTransformEffect = () => {
							...
							for (let k in skinSwitch.effects.transformEffects) {
								allEffects[k] = Object.assign({}, skinSwitch.effects
									.transformEffects[k])
							}
							pfqhUtils.getFoldsFiles(defaultTransformDir, function(file,
								path) {
								...
								initOptions(transEffectSelect, optionKeys, function(
									key, e) {
									let eff = allEffects[key]
									if (eff) {
										transItemScale.value = eff.scale ||
											0.5
										transItemDelay.value = eff.delay ||
											0.3
										transItemSpeed.value = eff.speed ||
											1
										transItemAngle.value = eff.angle ||
											0
										if (eff.x) {
			...
			skinSwitch.waitUntil(() => {
				...
				lib.init.js(skinSwitch.url, 'spine', function() {
					lib.init.js(skinSwitch.url + 'spine-lib', 'spine_4_0_64', function() {
						lib.init.js(skinSwitch.url + 'spine-lib', 'spine_3_8',
							function() {
								lib.init.js(skinSwitch.url + 'spine-lib',
									'spine_4_1',
									function() {
										lib.init.js(skinSwitch.url,
											'animations',
											function() {})
										lib.init.js(skinSwitch.url +
											'spine-lib', 'spine_3_5_35',
											function() {})
										lib.init.js(skinSwitch.url +
											'spine-lib', 'spine_3_7',
											function() {})
			...
			function editBoxInit() {
				...
				let changeInfoData = () => {
					...
					textInfoShow.innerHTML =
						`x: [${adjustX[0]}, ${x}]<br> y: [${adjustY[0]}, ${y}]<br>大小: ${scale}<br> 角度: ${angle}`
				}
				...
				let initBlackBg = () => {
							...
							buttons[i].listen(function() {
								switch (this.id) {
									case 'pfqh_qhly_bigedit4': {
										// 显示十字键辅助微调
										this._show = !adjustDirection || adjustDirection
					...
					function mousedownEvent(event) {
						if (event) {
							...
							if (this.posX) delete this.posX
							if (this.posY) delete this.posY
							if (event.touches && event.touches.length) blackbg._mousedown(event.touches[
								0].clientX, event.touches[0].clientY);
				...
				let selfLoopPlay = function(mode) {
					...
					skinSwitch.rendererOnMessage.addListener(player, 'debugChuKuang', function(e) {
						...
						if (player.isQhlx) {
							...
							let qhDivHeight = dynamicWrap.parentNode.parentNode
								.getBoundingClientRect().height
							let top = (bodyHeight - qhDivHeight) / 2
							canvas.style.top = -top + 'px'
							canvas.style.height = Math.round((decadeUI.get.bodySize().height /
								dynamicWrap.parentNode.parentNode
				...
				let saveToFile = function() {
					...
					if (saveKey) {
						...
						if (!player.isQhlx && currentMode === modes.daiji) {
							skinSwitch.saveSkinParams[player.name][saveKey] = Object.assign(skinSwitch
								.saveSkinParams[player.name][saveKey], modeData)
						} else {
						...
						// 修改千幻雷修版本的值
						if (skinSwitch.saveSkinParams[player.name][saveKey].qhlx) {
							decadeUI.dynamicSkin[player.name][saveKey].qhlx = skinSwitch.saveSkinParams[
								player.name][saveKey].qhlx
						}
					}
				}
				...
				lib.init.js(skinSwitch.url, 'pfqhUtils', function() {
					...
					if (lib.config[skinSwitch.configKey.showEditMenu]) {
						...
						ui.create.system('编辑动皮参数', function() {
							setTimeout(function() {
								if (!lib.config[skinSwitch.configKey
										.useDynamic]) {
									skinSwitchMessage.show({
										type: 'warning',
										text: '请先打开动皮功能',
								...
								if (game.me) {
									let player = game.me
									if (!player.dynamic || (!player.dynamic
											.primary)) {
					...
					if (lib.config[skinSwitch.configKey.czgEnable]) {
						...
						if (skinSwitch.qhly_hasExtension('如真似幻')) {
							...
							lib.init.js(skinSwitch.url + "spine-lib/pixi",
								"pixi_viewport_scrollbox")
				...
				lib.init.js(skinSwitch.url + 'component/live2d', 'live2dcubismcore.min', () => {
					...
					if (skinSwitch.qhly_hasExtension('如真似幻')) {
						lib.init.js(skinSwitch.url + 'component/live2d', 'pixi.min', () => {
							lib.init.js(skinSwitch.url + 'component/live2d', 'Live2dLoader',
								() => {
									// 读取l2d配置
									let initL2d = () => {
										let curVal = lib.config[skinSwitch.configKey
											.l2dSetting]
										if (curVal in pfqhLive2dSettings.models) {
											let base = Object.assign({},
												pfqhLive2dSettings.baseSetting)
											for (let k in pfqhLive2dSettings.models[
													curVal]) {
												base[k] = pfqhLive2dSettings.models[
													curVal][k]
											}
											base.role = lib.assetURL + base
												.basePath + base.role
											base.key = curVal
											skinSwitch.l2dLoader =
												new CustomLive2dLoader([
													base
												]);
										}
									}
									lib.init.js(skinSwitch.url, 'l2dSettings',
									function() {
										// 加载l2d
										...
										skinSwitch.qhly_checkFileExist(
											skinSwitch.path +
											'/l2dSettings.js', exists => {
												if (!exists) {
													game.readFile(skinSwitch
														.path +
														'/l2dSettings_示例.js',
														function(data) {
															game.writeFile(
						...
						lib.init.js(skinSwitch.url + "spine-lib/pixi", "pixi.min", () => {
							lib.init.js(skinSwitch.url + 'component/live2d', 'pixi.min',
							() => {
									lib.init.js(skinSwitch.url + 'component/live2d',
										'Live2dLoader', () => {
											// 读取l2d配置
											let initL2d = () => {
												let curVal = lib.config[
													skinSwitch.configKey
													.l2dSetting]
												if (curVal in pfqhLive2dSettings
													.models) {
													let base = Object.assign({},
														pfqhLive2dSettings
														.baseSetting)
		...
		config: {
			...
			"GXNR": {
				...
				"textMenu": function(node, link) {
					...
					switch (link) {
						case "xin":
							node.innerHTML = "<img style=width:100% src=" + lib.assetURL +
								"extension/皮肤切换/gengxin/1.06_更新.png>"
							break;
					}
				},
			},
